...
static void set_433buadrate(u32 buadrate);
/**********************************************/
...
uint16_t  Mode_Check_And_Choice(void)//uint16_t *buadtare
{
 ...
 {       
    ...
    uint8_t meter_rec_data[FromMeterSize];//ccdebu
   ...
   buadrate= Mode_Check_And_Choice(); //先判断是不是存在模式上的选择
   ...
   if(buadrate==FALSE) //模式选择没有开启
   {
    ...
    {     
        ...
        {
         set_433buadrate(2400);
         RS485_Send_Data(data,rece_data_num);
         set_433buadrate(1200);
         RS485_Send_Data(data,rece_data_num);
         return true ;
        }
        else
     ...
     HAL_UART_Receive_IT(&huart1,meter_rec_data,FromMeterSize);
    //发送数据到电表
    RS485_Send_Data(data,rece_data_num);
   ...
   while(1)
   {
    ...
    {   
      ...
      {
         HAL_UART_Receive_IT(&huart1,meter_rec_data,FromMeterSize);
         RS485_Send_Data(data,meter_rec_num);//发送一遍错误
         meter.rec_finish= false;
      }   
     else
        {
...
//    u8 rec_data[20];
    ...
    HAL_UART_Receive_IT(&huart1,out_data,FromMeterSize);
