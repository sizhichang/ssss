...
#include "app.h"
/*******************************************************/
Rec_Typedef  sim_rec_type[REC_QUE];
Cache_Typedef  sim_cache;
Rec_Typedef * sim_busy_rectype; 
Rec_Typedef * sim_free_rectype;
/*******************************************************/
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    {
        i++;
        if(i>5)  //因为在定时器使能时会进入中断，所以要在第二次进入该中断才说明发送数据完成
        {
          Wifi_Time_Callback();
          i = 0;
        }
